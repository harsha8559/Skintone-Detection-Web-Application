...
def result(request):
    ans=imgprocess(filees)
    context={'ans':ans}
    return render(request, 'skintone/result.html', context)
...
@csrf_exempt
def preview(request):
    print("Entere preview")
    if request.method== "POST":
        data= request.POST
        if data:
            print(data)
            if 'imagesubmit' in data:
                files = models.Fileupload.objects.filter(file_id='temp')
                fileupload = request.FILES.get('inputFile')
                if files.exists():
                    row = files[0]
                    row.fileupload = fileupload
                    row.save()

                else:
                    file_id ='temp'
                    fileupload = models.Fileupload.objects.create_fileupload(file_id=file_id,fileupload=fileupload)
                    fileupload.save() 
                    row = files[0]
                    row.fileupload = fileupload
                files = models.Fileupload.objects.filter(file_id='temp')
                ...
                global filees
                filees = row.fileupload.name
                str(filees)
                print('type',type(filees))
                print(filees)
                imgprocess(filees)
    render(request, 'skintone/result.html' , {})
...
def imgprocess(file):
    ...
    path = 'media/'+file
    print(path)
    (htap, galf) = detecter.generate( image_path = path, flag = 0)
...
class FaceCropper(object):
    ...
    CASCADE_PATH = "skintone/ML_Code/data/haarcascades/haarcascade_frontalface_default.xml"
